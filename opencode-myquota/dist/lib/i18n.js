...
const translations = {
    zh: {
        ...
        unknown: "未知",
        used: "已用",
        authError: (path, err) => `❌ 無法讀取認證檔：${path}\n錯誤：${err}`,
        apiError: (status, text) => `OpenAI API 請求失敗 (${status}): ${text}`,
        timeoutError: (seconds) => `請求逾時 (${seconds}秒)`,
        tokenExpired: "⚠️ OAuth 授權已過期，請在 OpenCode 中使用一次 OpenAI 模型以刷新授權。",
        noAccounts: "找不到任何已設定的帳號。\n\n支援的帳號類型：\n- GitHub Copilot\n- OpenAI (Plus/Team/Pro 訂閱)\n- Z.ai (Coding Plan)\n- MiniMax (Coding Plan)",
        queryFailed: "❌ 查詢失敗的帳號：\n",
        ...
        zhipuApiError: (status, text) => `智譜 API 請求失敗 (${status}): ${text}`,
        zaiApiError: (status, text) => `Z.ai API 請求失敗 (${status}): ${text}`,
        minimaxApiError: (status, text) => `MiniMax API 請求失敗 (${status}): ${text}`,
        ...
        billingPeriod: "計費週期",
        copilotApiError: (status, text) => `GitHub Copilot API 請求失敗 (${status}): ${text}`,
        copilotQuotaUnavailable: "⚠️ GitHub Copilot 配額查詢暫時不可用。\n" + "OpenCode 的新 OAuth 整合不支援存取配額 API。",
        copilotQuotaWorkaround: "解決方案：\n" +
    ...
    en: {
        ...
        hours: (n) => `${n}h`,
        minutes: (n) => `${n}m`,
        hourLimit: (h) => `${h}-hour limit`,
        dayLimit: (d) => `${d}-day limit`,
        ...
        unknown: "unknown",
        used: "Used",
        authError: (path, err) => `❌ Failed to read auth file: ${path}\nError: ${err}`,
        apiError: (status, text) => `OpenAI API request failed (${status}): ${text}`,
        timeoutError: (seconds) => `Request timeout (${seconds}s)`,
        tokenExpired: "⚠️ OAuth token expired. Please use an OpenAI model in OpenCode to refresh authorization.",
        noAccounts: "No configured accounts found.\n\nSupported account types:\n- GitHub Copilot\n- OpenAI (Plus/Team/Pro subscribers)\n- Z.ai (Coding Plan)\n- MiniMax (Coding Plan)",
        ...
        zhipuApiError: (status, text) => `Zhipu API request failed (${status}): ${text}`,
        zaiApiError: (status, text) => `Z.ai API request failed (${status}): ${text}`,
        minimaxApiError: (status, text) => `MiniMax API request failed (${status}): ${text}`,
        ...
        copilotApiError: (status, text) => `GitHub Copilot API request failed (${status}): ${text}`,
        copilotQuotaUnavailable: "⚠️ GitHub Copilot quota query unavailable.\n" + "OpenCode's new OAuth integration doesn't support quota API access.",
        copilotQuotaWorkaround: "Solution:\n" +
