...
- Google 風格建議摘要為單一實體行，並控制在 80 字元內。
- 若摘要後還有內容，摘要下一行需留空行。
- 避免只把函式名稱改寫成句子。
- 避免無資訊句型（例如：執行某功能）。
...
- Google Style：使用 `Args` / `Returns`（或 `Yields`）/ `Raises` 區段。
- `Examples` 區段可依專案規範決定是否強制。
- `@override` 方法可在不改變契約時省略 docstring。
- 同一專案應固定單一格式，不要混用。
